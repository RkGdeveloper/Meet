...
public class EMSOperation{
	...
	void displayEmployeeListDetails(ArrayList<EmployeeBean> empBeanList){
		...
		for(EmployeeBean beanEmp :empBeanList){
			...
			System.out.println(beanEmp.getEmpId()+" || "+beanEmp.getFirstName()+" || "+beanEmp.getLastName()+" || "+beanEmp.getDeptId()+" || "+beanEmp.getGrade()+" || "+beanEmp.getDesignationId());
		}
	}
	

	void displayEmployeeDetails(EmployeeBean beanEmp){
		System.out.println("ID  || FirstName || LastName  || Department || Grade || Designation \n");
		System.out.println(beanEmp.getEmpId()+" || "+beanEmp.getFirstName()+" || "+beanEmp.getLastName()+" || "+beanEmp.getDeptId()+" || "+beanEmp.getGrade()+" || "+beanEmp.getDesignationId());
	...
	public void updateOptions(EmployeeBean empB) throws EMSException {
		...
		int deptId = empB.getDeptId();
		
		deptB.setDeptId(deptId);
		
		deptB = adminServiceRef.getDepartmentDetailsById(deptId);
		System.out.println("Old Department Details "+deptB);
		
		
		int update =adminServiceRef.updateDepartmentName(dName,deptId);
		
		if(update>0){
			System.out.println("Department Name is Updated Successfully");
			System.out.println("New Department Details "+deptB);
...
IAdminManagementService service=new AdminManagementServiceImpl();
		...
		//{
			//if(service.validateEmpId1(empId))
		//	{
	...
	if(service.validateAge(age))
		...
		*/
		...
		//EmployeeBean employee = new EmployeeBean(empId,FirstName,LastName,df1,dt1,deptId,designationId,"M2",gender,salary,martialStatus,addressId,contactNo);
